/**
 * Import the July 2026 rota from the Excel sheet.
 *
 *   npm run seed-rota
 *
 * NOT idempotent, and deliberately so: it clears every shift and day note
 * between 2026-06-29 and 2026-08-02 and rewrites them from the data below, so
 * re-running it discards anything the boss changed on the Rota page inside that
 * window. It is a one-off import, which is why it is not part of
 * `scripts/deploy-bootstrap.mjs` and never runs on a deploy.
 */
...
const WEEKS: { dateKeys: string[]; rows: RowData[] }[] = [
  {
    // Week 1: Jun 29 - Jul 5
    ...
    rows: [
      ...
      { rowName: "New team member", shifts: ["Closed", "Closed", "", "", "", "", ""] },
      { rowName: "Lily", shifts: ["Closed", "Closed", "", "", "", "", ""] },
      { rowName: "Liam", shifts: ["Closed", "Closed", "", "", "", "", ""] },
      { rowName: "Ali Edwards", shifts: ["Closed", "Closed", "", "", "", "", ""] }
    ]
  },
  {
    // Week 2: Jul 6 - Jul 12
  ...
  },
  {
    ...
    rows: [
      ...
      { rowName: "New team member", shifts: ["Closed", "Closed", "", "", "", "", ""] },
      { rowName: "Lily", shifts: ["Closed", "", "", "", "", "", ""] },
      { rowName: "Liam", shifts: ["Closed", "", "", "", "", "", ""] },
      { rowName: "Ali Edwards", shifts: ["Closed", "", "", "", "", "", ""] }
    ]
  },
  {
    // Week 5: Jul 27 - Aug 2
...
function parseSingleShift(t: string): ShiftParsed | null {
  ...
  if (lower === "closed") {
    return { kind: ShiftKind.CLOSED };
  }
  if (lower === "unavailable") {
    return { kind: ShiftKind.UNAVAILABLE };
  }
  if (lower.startsWith("holiday")) {
    return { kind: ShiftKind.HOLIDAY, notes: val };
  }
  if (lower.startsWith("back ")) {
    return { kind: ShiftKind.UNAVAILABLE, notes: val };
  }

  // Working shifts mappings
  if (lower.includes("7-11pm")) {
    const notes = lower.includes("tbc") ? "TBC" : null;
    return { kind: ShiftKind.WORKING, startMinutes: 19 * 60, endMinutes: 23 * 60, notes };
  }
  ...
  if (lower.includes("4-10pm")) {
    const notes = lower.includes("tbc") ? "TBC" : null;
    return { kind: ShiftKind.WORKING, startMinutes: 16 * 60, endMinutes: 22 * 60, notes };
  }
  if (lower.includes("6-10pm")) {
    const notes = lower.includes("tbc") ? "TBC" : null;
    return { kind: ShiftKind.WORKING, startMinutes: 18 * 60, endMinutes: 22 * 60, notes };
  }
...
function getStaffNameAndSlot(rowName: string): { name: string; slot: number } {
  ...
  // Clean custom matches for DB values
  ...
  if (mappings[lookupKey]) {
    name = mappings[lookupKey];
  }
...
async function main() {
  console.log("Starting July 2026 Rota Seeder...");
  
  // 1. Ensure staff members exist and match DB
  ...
  // 2. Add any missing staff
  const staffToCreate = ["Harry", "Alex Newland", "Dan", "Felix", "Skye", "Dilan", "New team member", "Lily", "Liam", "Ali Edwards"];
  for (const name of staffToCreate) {
    if (!staffMap[name]) {
      const created = await prisma.staffMember.create({
        data: { name, active: true, sortOrder: staffToCreate.indexOf(name) }
      });
  ...
  // 3. Clear existing shifts for the target date ranges to prevent duplication
  const startRange = parseDateKey("2026-06-29");
  const endRange = parseDateKey("2026-08-02");
  ...
  console.log(`Cleared ${deletedShifts.count} existing shifts in July range.`);

  // 4. Clear and populate day notes
  const deletedNotes = await prisma.dayNote.deleteMany({
    where: {
      date: {
        gte: startRange,
        lte: endRange
  ...
  console.log(`Cleared ${deletedNotes.count} day notes.`);
  
  for (const n of DAY_NOTES) {
    const d = parseDateKey(n.dateKey);
    await prisma.dayNote.create({
      data: { date: d, note: n.note }
    });
  }
  console.log(`Seeded ${DAY_NOTES.length} day notes.`);
  ...
  for (const week of WEEKS) {
    for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
      ...
      for (const row of week.rows) {
        ...
        if (!staffId) {
          console.warn(`Staff member ${name} not found in map, skipping shift.`);
          continue;
        }
        ...
        for (let sIdx = 0; sIdx < parsedShifts.length; sIdx++) {
          ...
          const activeSlot = sIdx === 0 ? slot : slot + 1; // if it was split in a single cell, put second part in slot+1
  ...
  console.log(`Success! Seeded ${count} shifts for July 2026.`);
}
